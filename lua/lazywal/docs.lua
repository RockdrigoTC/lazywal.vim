-- LazyWal.vim/lua/lazywal/docs.lua
local Docs = require("lazy.docs")
local Groups = require("lazywal.groups")
...
function M.extras()
  local Extra = require("lazywal.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/lazywal/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/lazywal/config.lua", "\n(--@class lazywal%.Config.-\n})")
